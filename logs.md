...
- Added `scripts/process_courses.py` to enrich JSON with crosslist metadata (`crosslist`, `lower_crosslist`, `total_seats`, `total_enrollment`) based on matching time/days/instructor. Outputs go to `data/processed/` with an optional combined processed file.
...
- Clarified crosslist criteria and made `lower_crosslist` true only for the lowest course number within a crosslisted group (or for courses with no siblings).
